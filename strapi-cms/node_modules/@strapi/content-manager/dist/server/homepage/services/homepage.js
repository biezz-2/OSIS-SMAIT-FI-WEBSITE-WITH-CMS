...
const createHomepageService = ({ strapi })=>{
    ...
    const getRegisteredContentType = (uid)=>{
        const contentType = strapi.contentTypes[uid];
        if (contentType === undefined) {
            strapi.log.warn(`Skipping homepage content type "${uid}" because it is no longer registered.`);
            return undefined;
        }
        return contentType;
    };
    ...
    const getPermittedContentTypes = async ()=>{
        ...
        return [
            ...new Set(readPermissions.map((permission)=>permission.subject).filter((subject)=>{
                if (!subject) {
                    return false;
                }
                const contentType = strapi.contentTypes[subject];
                const contentTypeOptions = contentType?.pluginOptions?.['content-manager'];
    ...
    const getContentTypesMeta = (allowedContentTypeUids, configurations)=>{
        return allowedContentTypeUids.reduce((acc, uid)=>{
    ...
    return {
        ...
        async queryLastDocuments (additionalQueryParams, draftAndPublishOnly) {
            ...
            const allowedContentTypeUids = draftAndPublishOnly ? permittedContentTypes.filter((uid)=>{
                const contentType = getRegisteredContentType(uid);
                return contentType !== undefined && strapiUtils.contentTypes.hasDraftAndPublish(contentType);
