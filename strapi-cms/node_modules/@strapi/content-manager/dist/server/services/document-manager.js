...
const documentManager = ({ strapi })=>{
     ...
     */ async exists (uid, id) {
        ...
        async countDraftRelations (id, uid, locale) {
            ...
            return draft.sumDraftCounts(strapi, document, uid);
        },
        async countManyEntriesDraftRelations (documentIds, uid, locale) {
            ...
            const counts = await Promise.all(entities.map((entity)=>draft.sumDraftCounts(strapi, entity, uid)));
            return counts.reduce((total, entityCounts)=>total + entityCounts.unpublishedRelations + entityCounts.draftM2mLinks, 0);
        }
    };
};
