...
const newEngine = (params)=>{
   ...
   */ const evaluate = async (params)=>{
        ...
        const removeInvalidConditions = ___default.default.filter((condition)=>___default.default.isFunction(condition.handler));
        const evaluateConditions = (conditions)=>{
            return Promise.all(conditions.map(async (condition)=>({
                    condition,
                    result: await condition.handler(___default.default.merge(options, {
                        permission: ___default.default.cloneDeep(permission)
                    }))
                })));
        };
        const removeInvalidResults = ___default.default.filter(({ result })=>___default.default.isBoolean(result) || ___default.default.isObject(result));
