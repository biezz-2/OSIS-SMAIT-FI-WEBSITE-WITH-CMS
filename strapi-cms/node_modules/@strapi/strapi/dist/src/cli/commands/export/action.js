 ...
 */ var action = (async (opts)=>{
    ...
    dataTransfer$1.normalizeTransferFilterOptions(opts);
    const strapi = await dataTransfer$1.createStrapiInstance();
    dataTransfer$1.validateContentTypeTransferOptionsForStrapi(opts, strapi);
    ...
    const engine = dataTransfer.engine.createTransferEngine(source, destination, {
        ...
        transforms: dataTransfer$1.buildTransferTransforms(opts)
    });
    ...
    progress.on('transfer::start', async ()=>{
        ...
        dataTransfer$1.logTransferFilterSummary({
            exclude: opts.exclude,
            only: opts.only,
    ...
    try {
        ...
        if ((opts.format ?? 'tar') === 'dir') {
            const metadataPath = path__default.default.join(outFile, 'metadata.json');
            if (!await fse__default.default.pathExists(metadataPath)) {
                throw new dataTransfer.engine.errors.TransferEngineTransferError(`Export directory was not created correctly "${outFile}"`);
            }
        } else if (!await fse__default.default.pathExists(outFile)) {
 ...
 */ const createDestinationProvider = (opts)=>{
    ...
    if (format === 'dir') {
        ...
        const dirPath = path__default.default.isAbsolute(filepath) ? filepath : path__default.default.resolve(process.cwd(), filepath);
