...
const exitMessageText = (process1, error = false)=>{
    ...
    if (!error) {
        return chalk__default.default.bold(chalk__default.default.green(`${processCapitalized} process has been completed successfully!`));
    }
...
const buildTransferTable = (resultData)=>{
    ...
    const table = new CLITable__default.default({
        head: [
            ...
            'Count',
            'Size'
        ].map((text)=>chalk__default.default.bold.blue(text))
    });
    let totalBytes = 0;
    let totalItems = 0;
    Object.keys(resultData).forEach((stage)=>{
        ...
        table.push([
            {
                hAlign: 'left',
                content: chalk__default.default.bold(stage)
            },
            {
                hAlign: 'right',
                content: item.count
        ...
        if (item.aggregates) {
            Object.keys(item.aggregates).sort().forEach((subkey)=>{
                ...
                table.push([
                    {
                        hAlign: 'left',
                        content: `-- ${chalk__default.default.bold.grey(subkey)}`
                    },
                    {
                        hAlign: 'right',
                        content: chalk__default.default.grey(subitem.count)
                    },
                    {
                        hAlign: 'right',
                        content: chalk__default.default.grey(`(${helpers.readableBytes(subitem.bytes, 1, 11)})`)
                    }
                ]);
            });
        }
    ...
    table.push([
        {
            hAlign: 'left',
            content: chalk__default.default.bold.green('Total')
        },
        {
            hAlign: 'right',
            content: chalk__default.default.bold.green(totalItems)
        },
        {
            hAlign: 'right',
            content: `${chalk__default.default.bold.green(helpers.readableBytes(totalBytes, 1, 11))} `
        }
    ]);
    return table;
};
...
const excludeOption = new commander.Option('--exclude <comma-separated data types>', `Exclude data: ${transferExcludePresetsHelp}`).argParser(commander$1.getParseListWithChoices(transferExcludePresetChoices, 'Invalid options for "exclude"'));
const onlyOption = new commander.Option('--only <command-separated data types>', `Include only these types (plus schemas): ${transferOnlyPresetsHelp}`).argParser(commander$1.getParseListWithChoices(transferDataTypes, 'Invalid options for "only"'));
const excludeContentTypesOption = new commander.Option('--exclude-content-types <comma-separated UIDs>', `Exclude content types from entities and links (e.g. ${UPLOAD_CONTENT_TYPE_UIDS.join(',')} to omit the media library; or use --exclude media-library to skip binaries and upload records — see issue #25008)`).argParser(commander$1.parseList);
const onlyContentTypesOption = new commander.Option('--only-content-types <comma-separated UIDs>', 'Transfer only these content types in entities and links (e.g. api::article.article)').argParser(commander$1.parseList);
...
const validateContentTypeTransferOptions = (command)=>{
    ...
    if (overlap.length > 0) {
        helpers.exitWith(1, `Content types may not be used in both "--exclude-content-types" and "--only-content-types". Found in both: ${overlap.join(',')}`);
    }
};
const assertKnownContentTypes = (uids, strapi1, flag)=>{
    ...
    if (unknown.length > 0) {
        helpers.exitWith(1, `Unknown content type(s) for ${flag}: ${unknown.join(', ')}`);
    }
};
...
const formatDiagnostic = (operation, verbose)=>{
    ...
    const getLogger = ()=>{
        if (!logger$1) {
            ...
            const absoluteLogPath = path__default.default.resolve(process.cwd(), logFileBasename);
 ...
 */ const getDiffHandler = (engine, { force, action })=>{
    return async (context, next)=>{
        ...
        Object.entries(context.diffs).forEach(([uid, diffs])=>{
            for (const diff of diffs){
                ...
                if (uid === 'plugin::review-workflows.workflow' || uid === 'plugin::review-workflows.workflow-stage' || endPath?.startsWith('strapi_stage') || endPath?.startsWith('strapi_assignee')) {
                    ...
                    engine.reportWarning(chalk__default.default.red(`${chalk__default.default.bold(path)} does not exist on destination`), source);
        ...
        if (workflowsStatus === 'added') {
            ...
            engine.reportWarning(chalk__default.default.red(`Review workflows feature does not exist on destination`), source);
...
const logTransferFilterSummary = (opts)=>{
    ...
    if (parts.length) {
        console.log(chalk__default.default.dim(`Transfer filters: ${parts.join('; ')}.`));
    }
    ...
    if (contentTypeParts.length) {
        console.log(chalk__default.default.dim(`Content type filters: ${contentTypeParts.join('; ')}.`));
    }
    if (opts.filesAutoExcluded) {
        console.log(chalk__default.default.dim('Skipping files stage: upload content types are not in transfer scope (plugin::upload.file, plugin::upload.folder).'));
    }
    if (shouldSkipStage(opts, 'files') && !shouldSkipStage(opts, 'content') && areUploadContentTypesInTransferScope(opts) && !opts.filesAutoExcluded) {
        console.log(chalk__default.default.dim('Note: Media library records (plugin::upload.file, plugin::upload.folder) are still transferred with the rest of your content (the entities stage). Sync upload binaries separately (e.g. rsync public/uploads).'));
    }
};
...
exports.UPLOAD_CONTENT_TYPE_UIDS = UPLOAD_CONTENT_TYPE_UIDS;
exports.abortTransfer = abortTransfer;
exports.areAllUploadContentTypesOutOfTransferScope = areAllUploadContentTypesOutOfTransferScope;
exports.areUploadContentTypesInTransferScope = areUploadContentTypesInTransferScope;
exports.buildTransferTable = buildTransferTable;
exports.buildTransferTransforms = buildTransferTransforms;
exports.createEntityFilter = createEntityFilter;
exports.createLinkFilter = createLinkFilter;
exports.createStrapiInstance = createStrapiInstance;
exports.excludeContentTypesOption = excludeContentTypesOption;
...
exports.logTransferFilterSummary = logTransferFilterSummary;
exports.normalizeTransferFilterOptions = normalizeTransferFilterOptions;
exports.normalizeTransferFilterOptionsHook = normalizeTransferFilterOptionsHook;
exports.onlyContentTypesOption = onlyContentTypesOption;
...
exports.validateContentTypeTransferOptions = validateContentTypeTransferOptions;
exports.validateContentTypeTransferOptionsForStrapi = validateContentTypeTransferOptionsForStrapi;
