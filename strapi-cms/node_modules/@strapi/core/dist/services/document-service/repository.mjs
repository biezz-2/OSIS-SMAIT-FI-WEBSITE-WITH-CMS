import { curry, assoc, omit, isEmpty, pick } from 'lodash/fp';
...
import { prepareCloneData, copyCloneRelationRows } from './utils/clone-relations.mjs';
...
const createContentTypeRepository = (uid, validator = entityValidator)=>{
   ...
   */ const checkUnrecognizedRootParams = (params, strict)=>{
    ...
    async function clone(opts = {}) {
        ...
        const clonedEntries = await async.map(entriesToClone, async (entryToClone)=>{
            ...
            const { data, relationsToCopy } = await prepareCloneData(originalData, queryParams.data, contentType, (modelUid)=>strapi.getModel(modelUid));
            ...
            const selectionQuery = transformParamsToQuery(uid, pickSelectionParams({
                ...queryParams,
                status: 'draft'
            }));
    ...
    async function publish(opts = {}) {
        ...
        const selfRelationsToSync = await load$2(uid, draftsToPublish, 'published');
    ...
    async function discardDraft(opts = {}) {
        ...
        const selfRelationsToSync = await load$2(uid, versionsToDraft, 'draft');
        // Delete old drafts
