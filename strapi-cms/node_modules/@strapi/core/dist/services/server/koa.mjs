...
import { errorMethodEntries } from './koa-methods.mjs';

const addCustomMethods = (app)=>{
    const delegator = delegate(app.context, 'response');
    /* errors */ for (const { code, statusName, methodName } of errorMethodEntries()){
        app.response[methodName] = function responseCode(response = statusName, details = {}) {
            const httpError = createError(code, response, {
                details
            });
...
export { createKoaApp as default };
