...
import { constants, isScalarAttribute, isDynamicZoneAttribute, isMorphToRelationalAttribute } from '../content-types.mjs';
...
const isMorphLikeAttribute = (attribute)=>isDynamicZoneAttribute(attribute) || isMorphToRelationalAttribute(attribute);
...
const flattenDotPopulatePaths = (populate)=>{
    ...
    return items.flatMap((item)=>item.split(',').map((segment)=>trim(segment)).filter(Boolean));
};
...
const validatePopulateDotPaths = (ctx, populate)=>{
    for (const path of flattenDotPopulatePaths(populate)){
        const segments = path.split('.').map((segment)=>trim(segment)).filter(Boolean);
...
const validatePopulate = asyncCurry(async (ctx, populate, includes)=>{
    ...
    if (isDotNotationPopulate(populate)) {
        ...
        if (includes?.populate?.includes('private')) {
            return traverseQueryPopulate(visitor$3, ctx)(populate);
        }
        return populate;
    }
    // Build the list of functions conditionally based on the include array
