...
const isMorphLikeAttribute = (attribute)=>contentTypes.isDynamicZoneAttribute(attribute) || contentTypes.isMorphToRelationalAttribute(attribute);
...
const isDotNotationPopulate = (populate)=>{
    if (populate === '*') {
        return false;
    }
    ...
    return Array.isArray(populate) && populate.length > 0 && populate.every((p)=>typeof p === 'string');
};
const flattenDotPopulatePaths = (populate)=>{
    const items = Array.isArray(populate) ? populate : [
        populate
    ];
    return items.flatMap((item)=>item.split(',').map((segment)=>fp.trim(segment)).filter(Boolean));
};
const validatePopulateDotPathSegments = ({ schema, getModel }, segments, pathPrefix)=>{
    if (!segments.length) {
        return;
    }
    ...
    if (attribute.type === 'component') {
        ...
        validatePopulateDotPathSegments({
            schema: getModel(attribute.component),
            getModel
        }, tail, fullPath);
        return;
    }
    if (attribute.type === 'relation') {
        ...
        validatePopulateDotPathSegments({
            schema: getModel(attribute.target),
            getModel
        }, tail, fullPath);
        return;
    }
    ...
    if (attribute.type === 'dynamiczone') {
        ...
        const candidates = (attribute.components ?? []).map((uid)=>getModel(uid)).filter((model)=>model?.attributes?.[nextSegment]);
        ...
        for (const componentSchema of candidates){
            ...
            try {
                if (!rest.length) {
                    ...
                    validatePopulateDotPathSegments({
                        schema: getModel(nestedAttribute.component),
                        getModel
                    }, rest, nextPath);
                    validated = true;
                    break;
                } else if (nestedAttribute?.type === 'relation' && !isMorphLikeAttribute(nestedAttribute) && nestedAttribute.target) {
                    validatePopulateDotPathSegments({
                        schema: getModel(nestedAttribute.target),
                        getModel
                    }, rest, nextPath);
                    validated = true;
                    break;
...
const validatePopulateDotPaths = (ctx, populate)=>{
    for (const path of flattenDotPopulatePaths(populate)){
        const segments = path.split('.').map((segment)=>fp.trim(segment)).filter(Boolean);
        validatePopulateDotPathSegments(ctx, segments, '');
    }
};
const validateMorphLikeNestedPopulate = (populateValue, { path })=>{
    // Keep in sync with convert-query-params polymorphic nested populate handling.
    if (!fp.isNil(populateValue) && populateValue !== '*') {
        utils.throwInvalidKey({
            key: 'populate',
            path
        });
...
const validatePopulate = utils.asyncCurry(async (ctx, populate, includes)=>{
    ...
    if (isDotNotationPopulate(populate)) {
        validatePopulateDotPaths({
            schema: ctx.schema,
            getModel: ctx.getModel
        }, populate);
        if (includes?.populate?.includes('private')) {
            return queryPopulate(throwPrivate, ctx)(populate);
        }
        return populate;
    }
    // Build the list of functions conditionally based on the include array
    ...
    functionsToApply.push(queryPopulate(async ({ key, path, value, schema, attribute, getModel, parent }, { set })=>{
        /**
           ...
           */ if (!parent?.attribute && attribute) {
            ...
            if (isMorphLikeAttribute(attribute) && fp.isObject(value) && 'populate' in value) {
                validateMorphLikeNestedPopulate(value.populate, {
                    path: path.raw
                });
            }
