...
const SSRF_BLOCK_LIST = new net__default.default.BlockList();
 ...
 */ const getFilenameFromUrl = (url, contentDisposition)=>{
    ...
    if (contentDisposition) {
        ...
        if (filenameMatch?.[1]) {
            ...
            return path__default.default.basename(decodeURIComponent(filenameMatch[1]));
        }
    }
    ...
    try {
        ...
        if (filename && filename.length > 0) {
            ...
            return path__default.default.basename(decodeURIComponent(filename));
        }
    } catch  {
 ...
 */ const fetchUrlToInputFile = async (url, tmpWorkingDirectory, sizeLimit)=>{
    ...
    try {
        const { address, family } = await dns__default.default.lookup(parsedUrl.hostname);
    ...
    // Write to temp file
    const tmpFilePath = path__default.default.join(tmpWorkingDirectory, filename);
    await fse__default.default.writeFile(tmpFilePath, buffer);
