 ...
 */ const determineAttributeStatus = (newAttr, oldAttr, oldSchema)=>{
    ...
    const newAttrWithoutStatus = omit__default.default(newAttr, [
        'status'
    ]);
    const oldAttrWithoutStatus = omit__default.default(oldAttr, [
        'status'
    ]);
    if (!isEqual__default.default(newAttrWithoutStatus, oldAttrWithoutStatus)) {
        return 'CHANGED';
    }
 ...
 */ const transformChatToCTB = (schema, oldSchema)=>{
    const singularName = pluralize__default.default.singular(schema.name).toLowerCase().replace(/ /g, '-');
    const pluralName = pluralize__default.default.plural(schema.name).toLowerCase().replace(/ /g, '-');
