...
const getPreviewPathname = (uid: string, document: any): string | null => {
  ...
  switch (uid) {
    case 'api::halaman.halaman': {
      ...
      if (['beranda', 'home', 'halaman-utama'].includes(slug)) return '/';
      if (['tentang-kami', 'about'].includes(slug)) return '/about';
      if (slug === 'sekbid') return '/sekbid';
      if (slug === 'program-kerja') return '/program-kerja';
      if (slug === 'anggota') return '/anggota';
      if (slug === 'galeri') return '/galeri';
      if (slug === 'media-sosial') return '/media-sosial';
      if (slug === 'edufest-infinity') return '/edufest-infinity';
      return `/${slug}`;
    }
  ...
  preview: {
    ...
    config: {
      ...
      async handler(uid: string, { documentId, locale, status }: { documentId: string; locale?: string; status?: string }) {
        ...
        let document = await strapi.documents(uid as any).findOne({
          documentId,
          status: (status as any) || 'draft',
        });

        if (!document) {
          document = await strapi.documents(uid as any).findOne({ documentId, status: 'draft' });
        }
        if (!document) {
          document = await strapi.documents(uid as any).findOne({ documentId, status: 'published' });
        }
