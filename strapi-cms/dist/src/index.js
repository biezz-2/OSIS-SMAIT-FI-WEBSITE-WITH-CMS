...
function setupDatabaseSync() {
    const syncScript = path_1.default.join(__dirname, '../../scripts/sync-db.js');
    // Run sync immediately on startup
    console.log('[Sync] Spawning initial database sync process...');
    (0, child_process_1.fork)(syncScript, [], { stdio: 'inherit' });
    ...
    setInterval(() => {
        ...
        const child = (0, child_process_1.fork)(syncScript, [], { stdio: 'inherit' });
        child.on('error', (err) => {
 ...
 */
async function setupRBAC(strapi) {
    ...
    if (!roleNames.includes('Chief Editor')) {
        await strapi.plugin('users-permissions').service('role').createRole({
            name: 'Chief Editor',
            description: 'Ketua/Wakil/Sekjen OSIS — Full CRUD semua content, publish/approve, manage Contributors.',
            type: 'chief_editor',
        });
    ...
    if (!roleNames.includes('Content Contributor')) {
        await strapi.plugin('users-permissions').service('role').createRole({
            ...
            description: 'Pengurus Sekbid — Create/Edit draft content miliknya, submit for review.',
    ...
    for (const role of roles) {
        if (role.name === 'Chief Editor') {
            ...
            await setContributorPermissions(strapi, role.id, contributorContentTypes, allContentTypes);
        }
    }
}
async function setChiefEditorPermissions(strapi, roleId, contentTypes) {
    const permissions = {};
...
async function setContributorPermissions(strapi, roleId, writableContentTypes, allContentTypes) {
    const permissions = {};
...
async function setupPublicPermissions(strapi) {
    const publicRole = await strapi
        ...
        .service('role')
        .find();
    const publicRoleEntry = publicRole.find((r) => r.type === 'public');
    if (!publicRoleEntry)
        return;
...
// ============================================
async function ensureDefaultMediaAssets(strapi) {
    var _a;
    ...
    try {
        ...
        const store = strapi.db.query('strapi_core_store_settings');
        if (store) {
            ...
            if (configRecord && configRecord.value) {
                ...
                if ((_a = val === null || val === void 0 ? void 0 : val.layouts) === null || _a === void 0 ? void 0 : _a.edit) {
                    const editLayout = val.layouts.edit;
                    const flatFields = editLayout.flat().map((item) => item.name);
    ...
    try {
        const existingAssets = await strapi.documents('api::media-asset.media-asset').findMany({
            fields: ['key'],
        });
        ...
        for (const ma of defaultMediaAssets) {
            if (!existingKeys.has(ma.key)) {
                ...
                if (created && created.documentId) {
                    await strapi.documents('api::media-asset.media-asset').publish({ documentId: created.documentId });
                }
            }
        }
    }
    catch (error) {
        strapi.log.error(`Failed to seed default media assets: ${error.message}`);
    }
}
async function seedInitialData(strapi) {
    ...
    for (const a of anggotaData) {
        const created = await strapi.documents('api::anggota-osis.anggota-osis').create({
            data: { ...a, periode: '2025-2026', status_aktif: 'aktif' },
        });
        if (created && created.documentId) {
...
async function seedHalamanUtama(strapi) {
    const countHalaman = await strapi.documents('api::halaman.halaman').count({});
    if (countHalaman > 0) {
        return;
    }
    ...
    for (const h of halamanData) {
        const created = await strapi.documents('api::halaman.halaman').create({
            data: h,
        });
        if (created && created.documentId) {
            await strapi.documents('api::halaman.halaman').publish({ documentId: created.documentId });
        }
    }
    strapi.log.info(`  ✅ Created ${halamanData.length} Halaman Utama entries`);
}
async function publishExistingDrafts(strapi) {
    const contentTypes = [
        'api::sekbid.sekbid',
        'api::program-kerja.program-kerja',
        'api::anggota-osis.anggota-osis',
...
async function seedEdufestData(strapi) {
    try {
        // 1. Seed Edufest Config
        const configCount = await strapi.documents('api::edufest-config.edufest-config').count({});
        if (configCount === 0) {
            await strapi.documents('api::edufest-config.edufest-config').create({
                data: {
        ...
        const divCount = await strapi.documents('api::edufest-division.edufest-division').count({});
        if (divCount === 0) {
            const committeeData = [
                {
                    name: "Panitia Inti",
            ...
            for (const d of committeeData) {
                const createdDiv = await strapi.documents('api::edufest-division.edufest-division').create({
                    data: {
                        name: d.name,
                        slug: d.slug,
                        type: d.type,
                ...
                if (createdDiv && createdDiv.documentId) {
                    await strapi.documents('api::edufest-division.edufest-division').publish({ documentId: createdDiv.documentId });
                    for (const m of d.members) {
                        const createdMem = await strapi.documents('api::edufest-member.edufest-member').create({
                            data: {
                                name: m.name,
                        ...
                        if (createdMem && createdMem.documentId) {
                            await strapi.documents('api::edufest-member.edufest-member').publish({ documentId: createdMem.documentId });
                        }
                    }
                }
            }
        ...
        const timeCount = await strapi.documents('api::edufest-timeline.edufest-timeline').count({});
        if (timeCount === 0) {
            const timelineData = [
                {
                    year: "2017",
            ...
            for (const t of timelineData) {
                const createdT = await strapi.documents('api::edufest-timeline.edufest-timeline').create({
                    data: t
                });
                if (createdT && createdT.documentId) {
                    await strapi.documents('api::edufest-timeline.edufest-timeline').publish({ documentId: createdT.documentId });
                }
            }
