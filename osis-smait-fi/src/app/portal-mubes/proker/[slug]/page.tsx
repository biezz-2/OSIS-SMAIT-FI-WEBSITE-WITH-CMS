...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `Sidang LPJ Proker: ${slug} | Portal MUBES XXI`,
    robots: {
      index: false,
      follow: false,
      nocache: true,
...
export default async function MubesProkerDetailPage({ params }: PageProps) {
  const access = await getMubesAccess();

  // Proteksi mutlak: Hanya peserta sidang MUBES yang diizinkan mengakses halaman ini
  if (!access.allowed) {
  ...
  // 1. Ambil data publik proker dari Strapi
  const prokerData = await fetchProgramKerjaFromStrapi(slug);
  if (!prokerData) {
    notFound();
  }

  // 2. Ambil data rahasia LPJ dari Strapi via internal query
