...
import { fetchMediaAssetByKey } from "@/lib/strapi";
...
export default function AudioManager({ isLoading = false }: AudioManagerProps) {
    ...
    const [audioSrc, setAudioSrc] = useState("/sounds/Aidentity.mp3");
    const audioRef = useRef<HTMLAudioElement | null>(null);

    useEffect(() => {
        async function loadAudio() {
            const asset = await fetchMediaAssetByKey('bg-soundtrack', '/sounds/Aidentity.mp3');
            if (asset?.src) setAudioSrc(asset.src);
        }
        loadAudio();
    }, []);
    ...
    }, [isLoading, volume, hasInteracted, audioSrc]);
