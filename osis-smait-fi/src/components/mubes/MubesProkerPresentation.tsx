...
interface MubesProkerPresentationProps {
  ...
  /** modal = overlay di portal; full = halaman utuh /portal-mubes/proker/[slug] */
  variant: 'modal' | 'full';
  role?: string | null;
  onClose?: () => void;
...
export default function MubesProkerPresentation({
  ...
  const shellClass = isFull
    ? 'bg-white dark:bg-slate-900 w-full min-h-[70vh] rounded-none sm:rounded-3xl overflow-hidden shadow-none sm:shadow-xl flex flex-col border-0 sm:border border-slate-200 dark:border-slate-800'
    : 'bg-white dark:bg-slate-900 w-full max-w-5xl max-h-[90vh] rounded-3xl overflow-hidden shadow-2xl flex flex-col border border-slate-200 dark:border-slate-800';

  return (
    <div className={shellClass}>
      {/* Header */}
      <div className="p-6 bg-slate-950 text-white flex items-center justify-between border-b border-slate-800 gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-10 h-10 shrink-0 rounded-xl bg-amber-500/20 text-amber-400 flex items-center justify-center font-bold text-sm border border-amber-500/30">
            {proker.sekbid_nomor}
          </div>
          <div className="min-w-0">
            <span className="text-xs uppercase tracking-wider text-amber-400 font-semibold block">
              ...
              {isFull ? ' • Tampilan Penuh Sidang' : ''}
            </span>
            <h3 className="text-xl sm:text-2xl font-bold font-serif text-slate-100 truncate">
              {proker.judul}
            </h3>
          </div>
        </div>
        {!isFull && onClose ? (
          <button
            type="button"
            onClick={onClose}
            className="w-9 h-9 shrink-0 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition"
            aria-label="Tutup"
          >
            <X className="w-5 h-5" />
          </button>
      ...
      {/* Body */}
      <div
        className={`p-6 sm:p-8 text-slate-800 dark:text-slate-200 ${
          isFull ? '' : 'overflow-y-auto flex-1'
        }`}
      >
        {activeTab === 'overview' && (
            ...
            <div className="prose dark:prose-invert max-w-none bg-slate-50 dark:bg-slate-800/60 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 leading-relaxed text-sm whitespace-pre-line">
              {proker.teknis_pelaksanaan || (
                <div className="text-slate-400 italic">
                  Belum ada dokumen teknis rinci yang diunggah untuk program kerja ini. Pelaksanaan
                  mengacu pada TOR Seksi Bidang.
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === 'lpj' && (
          <div className="flex flex-col gap-6">
            ...
            {/* Full LPJ sections (body dinamis Strapi) — tampil di full & modal bila ada */}
            {proker.lpj && (
              <MubesLpjSection lpj={proker.lpj} role={role} />
            )}
      ...
      <div className="p-4 px-6 bg-slate-100 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 text-xs text-slate-500">
        <span className="hidden sm:inline">
          {isFull
            ? 'Tampilan penuh data sidang MUBES (bukan halaman visitor publik).'
            : 'Gunakan tab di atas untuk navigasi presentasi sidang.'}
        </span>
        <div className="flex items-center gap-2 justify-end flex-wrap">
          {isFull ? (
            <Link
              href="/portal-mubes"
              className="px-4 py-2 rounded-xl bg-slate-900 dark:bg-slate-800 text-white font-semibold hover:bg-slate-800 transition flex items-center gap-2"
            >
